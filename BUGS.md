...
| 154 | `rf-orm` | **Found by cycle-8 tests**: `where_in`/`where_not_in` emitted `col IN ?` binding the *entire* JSON array as one text value, so **every `IN` query returned 0 rows** against SQLite; fixed to expand the array into individual `?` placeholders (`col IN (?, ?, …)`) with empty-array sentinels; regression test added | latest |
| 155 | `rf-orm` | **Found by cycle-8 tests**: `whereNotBetween` split `col < min` into AND-wheres and `col > max` into OR-wheres, producing `(… AND col < min) OR col > max` when chained — silently including extra rows; fixed to emit an atomic `col NOT BETWEEN ? AND ?` in a single where slot; regression test added | latest |
| 156 | `rf-validation` | **Found by cycle-8 tests**: `BetweenLengthRule` used `s.len()` (UTF-8 byte count) instead of `s.chars().count()`, inconsistent with `MinLengthRule`/`MaxLengthRule` — e.g. `"café"` (4 chars / 5 bytes) was rejected by `BetweenLengthRule(4,4)` but accepted by min/max; fixed to count Unicode scalar values; regression test added | latest |
