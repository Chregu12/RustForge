        ...
        match self.inner.invoke(request).await {
            ...
            Err(err) => {
                ...
                let app_error = match err.clone() {
                    ApplicationError::CommandNotFound(cmd) => rf_plugins::AppError::new(
                        "COMMAND_NOT_FOUND",
                        format!("Command `{}` not found", cmd),
                    ),
                    ApplicationError::CommandAlreadyRegistered(cmd) => {
                        rf_plugins::AppError::new(
                            "COMMAND_ALREADY_REGISTERED",
                            format!("Command `{}` already registered", cmd),
                        )
                    }
                    ApplicationError::CommandExecution(msg) => rf_plugins::AppError::new(
                        "COMMAND_EXECUTION_ERROR",
                        format!("Command execution failed: {}", msg),
                    ),
                    ApplicationError::StorageError(msg) => {
                        rf_plugins::AppError::new("STORAGE_ERROR", msg)
                    }
                    ApplicationError::RegistryCorrupted => rf_plugins::AppError::new(
                        "REGISTRY_CORRUPTED",
                        "Registry corrupted: lock poisoned",
                    ),
                    ApplicationError::LockPoisoned(msg) => rf_plugins::AppError::new(
                        "LOCK_POISONED",
