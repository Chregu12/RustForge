...
impl FriendlyError for DatabaseError {
    fn friendly_message(&self) -> String {
        match &self.kind {
            ...
            DatabaseErrorKind::Query { .. } => "A database query failed to execute.".to_string(),
            DatabaseErrorKind::Migration { .. } => "Database migration failed.".to_string(),
            DatabaseErrorKind::Transaction { .. } => "Database transaction failed.".to_string(),
    ...
    fn suggested_fixes(&self) -> Vec<String> {
        match &self.kind {
            ...
            DatabaseErrorKind::PoolExhausted { max_connections } => vec![
                format!(
                    "Increase pool size in config (current: {})",
                    max_connections
                ),
...
impl FriendlyError for AuthenticationError {
    fn friendly_message(&self) -> String {
        match self.kind {
            ...
            AuthenticationErrorKind::UserNotFound => "User account not found.".to_string(),
            AuthenticationErrorKind::AccountLocked => "This account has been locked.".to_string(),
    ...
    fn suggested_fixes(&self) -> Vec<String> {
        match self.kind {
            ...
            AuthenticationErrorKind::TokenExpired => {
                vec!["Log in again to get a new session".to_string()]
            }
...
mod tests {
    ...
    fn test_database_error_suggested_fixes() {
        ...
        assert!(fixes
            .iter()
            .any(|f| f.contains("systemctl") || f.contains("psql")));
    }

    #[test]
    fn test_validation_error_friendly() {
        let err = ValidationError::new("email", "Invalid format").with_value("not-an-email");
        assert!(err.friendly_message().contains("email"));
        assert!(err
            .suggested_fixes()
            .iter()
            .any(|f| f.contains("not-an-email")));
    }
