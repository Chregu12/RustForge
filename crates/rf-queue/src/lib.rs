...
pub use job::{Job, JobHandler, JobMetadata};
pub use memory::MemoryQueue;

// Re-export `async_trait` so code generated by `#[derive(Job)]` can reference it
// as `::rf_queue::async_trait` without the downstream crate depending on it.
pub use async_trait;

/// Derive the [`Job`] trait for a struct, generating `job_type` (the struct
/// name), the `queue`/`max_retries`/`timeout`/`priority` accessors (from an
/// optional `#[job(..)]` attribute) and wiring the body to your [`JobHandler`]
/// impl. See [`JobHandler`] for a full example.
pub use rf_macros::Job;
pub use queue::Queue;
