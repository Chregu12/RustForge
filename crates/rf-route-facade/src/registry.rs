...
use rf_routing::{Route as RfRoute, RouteRegistry, NamedRoute};
...
impl GlobalRouter {
    ...
    pub fn url(&self, name: &str, params: &HashMap<String, String>) -> Option<String> {
        let params_converted: HashMap<String, rf_routing::ParamValue> = params
            .iter()
            .map(|(k, v)| (k.clone(), rf_routing::ParamValue::String(v.clone())))
            .collect();
...
///
/// ```rust
/// use rf_route_facade::global_router;
///
