...
mod integration_tests {
    ...
    async fn test_complete_authorization_code_flow() {
        ...
        let tokens = server
            .exchange_authorization_code(&client, &auth_code, None)
            .await
            .unwrap();
    ...
    async fn test_pkce_flow_public_client() {
        ...
        let tokens = server
            .exchange_authorization_code(&client, &auth_code, Some(code_verifier.to_string()))
            .await
            .unwrap();
    ...
    async fn test_client_credentials_flow() {
        ...
        let result = server
            .issue_client_credentials_token(&client, vec!["users:read".to_string()])
            .await;
        ...
        let result = server
            .issue_client_credentials_token(&client, vec!["invalid:scope".to_string()])
            .await;
    ...
    async fn test_invalid_grant_type() {
        ...
        let result = server
            .issue_client_credentials_token(&client, vec!["read".to_string()])
            .await;
    ...
    async fn test_authorization_code_exchange_validation() {
        ...
        let result = server
            .exchange_authorization_code(&client2, &auth_code, None)
            .await;
    ...
    async fn test_password_grant_flow() {
        ...
        let user_id = Uuid::new_v4();
        let tokens = server
            .issue_password_token(&client, user_id, vec!["users:read".to_string()])
            .await
            .unwrap();
