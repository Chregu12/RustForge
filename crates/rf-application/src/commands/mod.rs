...
// Note: DownCommand and UpCommand removed - use rf_maintenance::{AppDownCommand, AppUpCommand} instead
...
impl BootstrapCommands {
    pub fn register_all(registry: &CommandRegistry) -> Result<(), ApplicationError> {
        ...
        // Note: DownCommand and UpCommand are now provided by rf_maintenance crate
        ...
        let app_down = Arc::new(rf_maintenance::AppDownCommand);
        ...
        let env_validate = Arc::new(rf_env::EnvValidateCommand);
        registry.register(env_validate)?;

        let env_reload = Arc::new(rf_env::EnvReloadCommand);
        registry.register(env_reload)?;

        let asset_publish = Arc::new(rf_assets::AssetPublishCommand);
