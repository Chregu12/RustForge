...
impl MakeModelCommand {
    ...
    fn compute_plan(&self, ctx: &CommandContext) -> Result<(String, GeneratorPlan), CommandError> {
        ...
        // The Model! DSL derives the table name as plural snake_case (e.g. "Post" → "posts").
        // Match that exact convention so the generated migration creates the table that
        // Model!() will actually target — otherwise migrate + create!/find! fail at runtime.
        let table = pluralize(&slug);
        ...
        let plan = GeneratorPlan {
            artifacts: vec![
                ...
                GeneratedArtifact {
                    ...
                    description: format!("Migration: Tabelle `{table}` erstellen"),
                    preview: Some(model_migration_up_sql(&table)),
                },
                GeneratedArtifact {
                    path: format!("{migration_dir}/down.sql"),
                    description: format!("Rollback: Tabelle `{table}` entfernen"),
                    preview: Some(migration_down_sql(&table)),
                },
            ],
            summary: Some(format!(
...
fn model_template(struct_name: &str) -> String {
    // Emit the canonical Model!() DSL (from rf::prelude) — NOT the legacy
    // #[model] attribute macro, which panics because SeaORM's DeriveEntityModel
    // requires the struct to be named `Model`, not the user-supplied name.
    // Model!() handles the inner entity wiring correctly and is the tested,
    // shipped pattern used by taskflow / blog-slice / rest-crud-resource.
    //
    // At least one real field (name: String) is emitted so that create!/find!
    // work immediately on freshly-generated code without any hand-editing.
    // The generated migration (model_migration_up_sql) includes the matching
    // `name TEXT NOT NULL` column, keeping the two artefacts in sync.
    //
    // #![allow(dead_code)] suppresses warnings on the generated types until
    // the developer wires them into controllers/queries.
    format!(
        "#![allow(dead_code)]\nuse rf::prelude::*;\n\n// `id`, `created_at`, and `updated_at` are auto-generated by the Model! DSL.\n// Add or replace fields below; the companion migration already includes `name`.\nModel!({struct_name} {{\n    name: String,\n    // Add more fields here, e.g.:\n    // email: String,\n    // body: Option<String>,\n}});\n"
    )
}

/// Naive pluralisation that mirrors the convention used by the Model! macro:
/// `format!("{}s", to_snake_case(name))`. Applying it to the slug (which is
/// already snake_case) produces the same table name that Model! will target.
fn pluralize(s: &str) -> String {
    format!("{}s", s)
}

/// Migration skeleton for a model generated by `make:model`.
/// Includes a sample `name TEXT NOT NULL` column that matches the `name: String`
/// field emitted by `model_template`, so create!/find! work with NO hand-editing.
fn model_migration_up_sql(table: &str) -> String {
    format!(
...
fn controller_template(name: &str) -> String {
    format!(
        "#![allow(dead_code)]\nuse axum::extract::State;\nuse rf_api::{{AppJson, ApiResult, AppState, JsonResponse}};\nuse rf_plugins::ValidationRules;\nuse serde::{{Deserialize, Serialize}};\n\n#[derive(Clone)]\npub struct {name}Controller;\n\n#[derive(Serialize)]\npub struct {name}Resource {{\n    pub id: i64,\n}}\n\n#[derive(Serialize, Deserialize)]\npub struct Create{name}Payload {{\n    pub name: String,\n}}\n\npub async fn index(State(_state): State<AppState>) -> ApiResult<Vec<{name}Resource>> {{\n    Ok(JsonResponse::ok(vec![]))\n}}\n\npub async fn store(\n    State(state): State<AppState>,\n    payload: AppJson<Create{name}Payload>,\n) -> ApiResult<{name}Resource> {{\n    let rules = ValidationRules {{\n        rules: serde_json::json!({{\n            \"required\": [\"name\"],\n            \"fields\": {{\n                \"name\": {{ \"min_length\": 3 }}\n            }}\n        }}),\n    }};\n    payload.validate(&state, rules).await?;\n    let _input = payload.into_inner();\n\n    Ok(JsonResponse::created({name}Resource {{ id: 1 }}))\n}}\n"
    )
}

fn routes_template(_name: &str, slug: &str) -> String {
    // axum 0.8: `get(handler)` / `post(handler)` return `MethodRouter<S>`, NOT
    // `Route<S>`. The previous template imported `Route` from `axum::routing` and
    // used it as the parameter type of a `with_optional_middleware` wrapper, which
    // caused four type-mismatch errors on every generated scaffold.
    //
    // Fixed: pass `get(handler)` / `post(handler)` directly to `.route()`, which
    // accepts `MethodRouter<S>`.  The `AppState` type param is already carried by
    // `app_router()` → `Router<AppState>`, so handler state-extraction still works.
    // To add per-route middleware, chain `.route_layer(from_fn_with_state(…))` on
    // the returned `AppRouter` instead of wrapping individual handlers.
    format!(
        "use axum::routing::{{get, post}};\nuse rf_api::{{app_router, AppRouter}};\n\npub fn routes() -> AppRouter {{\n    app_router()\n        .route(\n            \"/{slug}\",\n            get(crate::app::http::controllers::{slug}_controller::index),\n        )\n        .route(\n            \"/{slug}\",\n            post(crate::app::http::controllers::{slug}_controller::store),\n        )\n    // To add route-level middleware:\n    // .route_layer(axum::middleware::from_fn_with_state(\n    //     state.clone(),\n    //     crate::app::http::middleware::ensure_token::ensure_token,\n    // ))\n}}\n"
    )
}
...
fn register_controller_modules(ctx: &CommandContext, slug: &str) -> Result<(), CommandError> {
    ...
    // Wire the new route module into the kernel's app_routes() automatically,
    // so the developer does not have to add `.merge(...)` by hand.
    wire_route_into_kernel(ctx, slug)?;
...
/// Wire a newly generated route module into the HTTP kernel's `app_routes()`.
///
/// After `make:controller Foo` the route file `app/http/routes/foo.rs` exists and
/// `pub mod foo;` is registered in `routes/mod.rs`, but the developer would
/// otherwise have to manually add `.merge(crate::app::http::routes::foo::routes())`
/// to `kernel.rs`. This function does that automatically (idempotent).
fn wire_route_into_kernel(ctx: &CommandContext, slug: &str) -> Result<(), CommandError> {
    ...
    let route_call = format!("crate::app::http::routes::{}::routes()", slug);
    let mut content = fs::read_to_string(&kernel_path)
        .map_err(|err| CommandError::Other(err.into()))?;

    // Idempotent: skip if the merge call is already present.
    if content.contains(&route_call) {
        return Ok(());
    }

    // Find the `app_router()` line inside `app_routes()` and insert the merge
    // call immediately after it so the chain reads:
    //   app_router()
    //       .merge(crate::app::http::routes::{slug}::routes())
    let anchor = "\n    app_router()";
    if let Some(pos) = content.find(anchor) {
        let insert_at = pos + anchor.len();
        let merge_line = format!(
            "\n        .merge({})",
            route_call
        );
        content.insert_str(insert_at, &merge_line);
        fs::write(&kernel_path, &content)
            .map_err(|err| CommandError::Other(err.into()))?;
    }
...
fn login_request_template() -> String {
    r#"use rf_plugins::ValidationRules;
use serde::Deserialize;
...
mod tests {
    ...
    fn slugify_preserves_underscores() {
        // Bug fix: underscores in input must be preserved (they were silently dropped).
        assert_eq!(slugify("blog_post"), "blog_post");
        assert_eq!(slugify("blog__post"), "blog_post"); // double underscore collapsed
        assert_eq!(slugify("create_posts_table"), "create_posts_table");
    }

    #[test]
    fn pluralize_appends_s() {
        // Must match the convention used by the Model! macro in rf-macros:
    ...
    #[test]
    fn model_migration_table_name_matches_model_dsl() {
        // The Model! macro uses `format!("{}s", to_snake_case(name))`.
        // `slugify` produces the same snake_case as `to_snake_case`, so
        // `pluralize(slugify(name))` must equal what Model! computes.
        assert_eq!(pluralize(&slugify("Post")), "posts");
        assert_eq!(pluralize(&slugify("BlogPost")), "blog_posts");
        assert_eq!(pluralize(&slugify("UserProfile")), "user_profiles");
    }

    #[test]
    fn model_template_contains_sample_field() {
        let tmpl = model_template("Post");
        // The template must emit a real (uncommented) field so create!/find! work
        // immediately on freshly-generated code with no hand-editing.
        assert!(
            tmpl.contains("name: String,"),
            "model template must include a real sample field; got:\n{tmpl}"
        );
    }

    #[test]
    fn model_migration_up_sql_uses_plural_table_and_name_column() {
        // The migration SQL generated for `make:model Post` must use "posts" (not
        // "post") and include a `name` column matching the sample model field.
        let sql = model_migration_up_sql("posts");
        assert!(
            sql.contains("CREATE TABLE IF NOT EXISTS posts"),
            "migration must target plural table name; got:\n{sql}"
        );
        assert!(
            sql.contains("name TEXT NOT NULL"),
            "migration must include sample name column; got:\n{sql}"
        );
    }

    #[tokio::test(flavor = "current_thread")]
    async fn make_model_migration_targets_plural_table() {
        // End-to-end check: make:model Post generates migration SQL that targets
        // "posts" (the same table that Model!(Post {}) will use at runtime).
        let command = MakeModelCommand::new();
        let mut harness = base_harness();
        harness.ctx.args = vec!["Post".into()];
        ...
        let up_sql = written
            .iter()
            .find(|(path, _)| path.ends_with("up.sql"))
            .expect("up.sql must be written");
        assert!(
            up_sql.1.contains("CREATE TABLE IF NOT EXISTS posts"),
            "up.sql must reference the PLURAL table 'posts', got:\n{}",
            up_sql.1
        );
        assert!(
            up_sql.1.contains("name TEXT NOT NULL"),
            "up.sql must include sample name column; got:\n{}",
            up_sql.1
        );
    }

    #[tokio::test(flavor = "current_thread")]
    async fn make_controller_wires_route_into_kernel() {
        // After make:controller, the generated kernel.rs must include a
        // .merge(crate::app::http::routes::account::routes()) call so the
        // developer does not have to add it manually.
        ...
        assert!(kernel_file.exists(), "kernel.rs must be created");

        let kernel_contents = fs::read_to_string(&kernel_file).expect("read kernel.rs");
        assert!(
            kernel_contents.contains("crate::app::http::routes::account::routes()"),
            "kernel.rs must wire the generated route module; got:\n{kernel_contents}"
        );
    }
