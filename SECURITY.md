...
- ~~**CSRF via form-body tokens**~~ — Resolved: the CSRF middleware parses the `_token` field from both `application/x-www-form-urlencoded` (cycle 6) and `multipart/form-data` (cycle 10) bodies — buffered and re-inserted so the downstream handler (including file uploads) still receives the payload — in addition to the `X-CSRF-TOKEN` header. Traditional HTML form and file-upload submissions are protected without JavaScript.
